...
export default function InteractiveJournal() {
  ...
  return (
    <div className="rounded-2xl border border-stone-200/40 dark:border-stone-800/60 bg-white/40 dark:bg-stone-900/40 backdrop-blur-xl p-6 md:p-8 shadow-sm">
      <div className="max-w-3xl mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="text-xs md:text-sm text-stone-500 dark:text-stone-400 mt-2 max-w-lg mx-auto">
        ...
        <div className="relative min-h-[160px] mb-8 overflow-hidden rounded-xl border border-stone-100 dark:border-stone-800 bg-stone-50/50 dark:bg-stone-950/40 p-5 flex flex-col justify-between">
          ...
          <form onSubmit={handleRelease} className="space-y-4">
            <div>
              ...
              <textarea
                ...
                className="w-full min-h-[90px] bg-transparent border-0 placeholder-stone-400 dark:placeholder-stone-600 text-stone-800 dark:text-stone-200 focus:outline-none focus:ring-0 text-sm md:text-base font-sans resize-none font-light py-1"
                disabled={isLettingGo}
              />
              <div className="text-right text-[10px] font-mono text-stone-400 dark:text-stone-600">
                {text.length}/280
              </div>
            </div>

            <div className="flex flex-wrap items-center justify-between gap-4 pt-3 border-t border-stone-100 dark:border-stone-800/80">
              <div className="flex flex-wrap items-center gap-1.5">
                <span className="text-[10px] font-mono text-stone-400 dark:text-stone-500 mr-1.5">Category:</span>
                {categories.map(cat => (
                  <button
                    key={cat}
                    type="button"
                    ...
                    className={`px-2.5 py-1 rounded-full text-[10px] font-mono transition-all cursor-pointer ${
                      category === cat 
                        ...
                        : 'bg-stone-100 text-stone-600 dark:bg-stone-900/60 dark:text-stone-400 hover:bg-stone-200/50 dark:hover:bg-stone-800/50'
                    }`}
                  >
                    {cat}
                  </button>
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-4 py-1.5 rounded-full text-xs font-mono tracking-tight transition-all cursor-pointer ${
                  text.trim() && !isLettingGo
                    ? 'bg-stone-900 text-white hover:bg-stone-800 dark:bg-stone-100 dark:text-stone-900 dark:hover:bg-white border border-stone-900 dark:border-stone-100'
                    : 'bg-stone-100 text-stone-400 dark:bg-stone-900 dark:text-stone-600 border border-transparent cursor-not-allowed'
                }`}
        ...
        <div className="mt-12 space-y-6">
          <div className="flex items-center justify-between border-b border-stone-100 dark:border-stone-900 pb-3">
            <h4 className="text-xs font-mono uppercase tracking-[0.1em] text-stone-500 dark:text-stone-400">
              ...
              {['All', ...categories].map(cat => (
                <button
                  ...
                  className={`px-2 py-0.5 rounded text-[10px] font-mono cursor-pointer transition-colors ${
                    filterCategory === cat
                      ? 'text-stone-900 dark:text-white font-medium bg-stone-150 dark:bg-stone-800/40'
              ...
              {filteredReflections.map((ref) => {
                ...
                return (
                  <motion.div
                    ...
                    className="p-5 rounded-xl border border-stone-100 dark:border-stone-800/60 bg-white/20 dark:bg-stone-950/20 flex flex-col justify-between space-y-4 hover:border-stone-200/60 dark:hover:border-stone-700/65 transition-all group"
                  >
                    <div>
                      <div className="flex items-center justify-between gap-2 mb-2">
                        <span className="text-[9px] font-mono uppercase px-2 py-0.5 rounded-full bg-stone-100 dark:bg-stone-900/80 text-stone-500 dark:text-stone-400">
                          {ref.category}
                        </span>
                      ...
                      <p className="text-xs md:text-sm text-stone-700 dark:text-stone-300 font-light leading-relaxed">
                        “{ref.text}”
                      </p>
                    </div>

                    <div className="flex items-center justify-between pt-3 border-t border-stone-100/60 dark:border-stone-900/40">
                      <button
                        onClick={() => handleReact(ref.id)}
                        className="flex items-center gap-1.5 text-[10px] font-mono text-stone-400 dark:text-stone-500 hover:text-rose-500 dark:hover:text-rose-400 cursor-pointer transition-colors group-hover:text-stone-500"
